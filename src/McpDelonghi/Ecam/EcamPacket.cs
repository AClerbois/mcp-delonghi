...
public static class EcamPacket
{
        ...
        int profile = 2, int intensity = 1, Dictionary<int, int>? overrides = null)
    {
        ...
        brewParams.AddRange([27, 1]); // IDX_LEN=1 (matches app behavior)

        if (isIced)
            brewParams.AddRange([31, 0]);  // ICED=0
        else if (isColdBrew)
            brewParams.AddRange([31, 3, 38, (byte)intensity]); // ICED=3 + INTENSITY
